...
migration-create:
	docker container exec -it backend sh -c "npm run typeorm:migration:create $(name)"
migration-generate:
	docker container exec -it backend sh -c "npm run typeorm:migration:generate $(name)"
migration-run:
...
schema-sync:
	docker container exec -it backend sh -c "npm run typeorm:schema:drop && npm run typeorm:migration:run && npm run typeorm:seed:run"